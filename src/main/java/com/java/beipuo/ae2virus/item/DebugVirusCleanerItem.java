...
public class DebugVirusCleanerItem extends Item {
    private final VirusClass virusClass;

    public DebugVirusCleanerItem(VirusClass virusClass, Properties properties) {
        super(properties);
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        int cleared = service.debugRemoveVirusAndClear(this.virusClass);
        PacketDistributor.sendToPlayer(player, new SyncInfectedKeysPacket(service.getInfectedKeys()));
        player.displayClientMessage(Component.translatable("message.ae2virus.debug_virus.cleaned",
                Component.translatable("virus_class.ae2virus." + this.virusClass.name().toLowerCase()), cleared)
