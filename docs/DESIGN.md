...
- 物品数量保持不变，但不可用于合成或取出。
- 物品有概率恢复，有概率继续恶化。
- 损坏状态只作为网络风险和修复流程的一部分，不再注册独立副产物材料。
...
- 不同病毒等级和侵染方式应有各自的时间上下限、倍率上限和训练上限，避免所有分组在后期收敛成同一种体验。
...
如果管道或储罐不能保留 Data Component，机器内部应优先使用数据流胶囊作为精确输入；普通流体管道只适合搬运未指定或已标准化的数据流。

当前实现状态：

- 数据流通过 `ae2virus:data_stream` AEKey 表示，显示单位为 MB。
- 数据流胶囊可以保存一个具体数据流目标，空胶囊和已填充胶囊使用同一个注册物品，通过内部数据区分。
- 数据流存储元件用于接入 AE 网络存储数据流，不再为每一种数据流注册独立物品或流体。
- 机器配方当前按 `data_stream_amount = 1000` 表示需要 1000 MB 数据流。
...
- 注册方块为 `ae2virus:virus_core_manufacturer`。
- 机器拥有 3 个物品输入槽、1 个数据流胶囊槽和 1 个输出槽。
- 当前基础配方输入为 AE 成型核心、破坏核心、病毒外壳和 1000 MB 数据流。
- 输出为携带数据流目标信息的 `ae2virus:virus_core`。
- 单次工序消耗 200 AE，机器优先使用自身内部能量，不足时从所在 AE 网络取能。

### 8.5 病毒组装机

病毒组装机用于把病毒核心、数据流和安全模块组装成可控病毒。

基础配方结构：

1. 输入病毒核心。
2. 输入带有物品信息的数据流。
3. 输入安全模块、数据库或其他配置允许的稳定化材料。
4. 消耗能量与时间。
5. 输出携带目标数据的病毒。
...
- 数据流决定病毒后续可以读取的目标。
- 病毒等级和侵染方式决定病毒的生产范围、产量上限、生产时间和是否允许生产黑名单物品。
- 病毒核心决定病毒可运行的平台和效率上限。
- 若核心等级低于数据流等级，组装失败或产出不稳定病毒。
- 高纯度数据流可以提高最终病毒的产出效率。
- T2 专一病毒可以获得黑名单生产权限，例如生产被整合包列入病毒生产黑名单的物品。

可控病毒不应默认继续感染玩家主网。它应被视为经过封装的生产单元，只能在专用繁殖机中运行。只有在配置允许或机器损坏、过载、断电等特殊情况下，它才可能重新变成网络威胁。

当前实现状态：

- 注册方块为 `ae2virus:virus_assembler`。
- 机器拥有 2 个物品输入槽、1 个数据流胶囊槽和 1 个输出槽。
- 当前基础配方输入为病毒核心和病毒外壳。
- 输出病毒根据病毒核心内部载荷决定分组：T1 基础、T2 融合、T2 特化、T2 专一、T3 规则或创造病毒。
- T2 特殊病毒暂不作为当前默认配方产物注册。
- 单次工序消耗 120 AE。
...
- 注册方块为 `ae2virus:virus_breeder`。
- 机器拥有 1 个物品输入槽、1 个数据流胶囊槽和 1 个输出槽。
- 当前基础配方接受病毒核心或已封装病毒作为输入，并消耗 1000 MB 匹配数据流。
- 每次成功工序为输入病毒载荷增加少量经验，输出仍为同一病毒或核心物品。
- 单次工序消耗 80 AE。
- 三类机器配方均注册为特殊配方，不进入原版配方书；JEI 使用专用分类显示。
- 开发运行环境需要加载完整 AE2WTLib，而不是只加载 AE2WTLib API，否则 AdvancedAE 的无线终端配方会缺少 `ae2wtlib:combine` 和 `ae2wtlib:upgrade` 序列化器。
...
- 玩家第一次意识到网络安全问题。
- 防火墙成为一个明确的中期目标。
- 首次杀毒后回收少量低纯度数据流，提示病毒系统也可以被研究利用。
...
- 数据流应是事故回收信息，不应让玩家通过反复感染主网获得无成本收益。
